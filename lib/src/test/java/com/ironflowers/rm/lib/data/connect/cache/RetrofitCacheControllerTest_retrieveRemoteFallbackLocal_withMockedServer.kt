...
class RetrofitCacheControllerTest_retrieveRemoteFallbackLocal_withMockedServer {
    ...
    fun remoteSuccess() {
        ...
        val testObserver = cacheController
                .retrieveRemoteFallbackLocal { cachingHeader -> exampleCachingApi.doSomeExampleGet(cachingHeader) }
                .test()
    ...
    fun remoteFailureLocalSuccess() {
        ...
        testObserver.assertValueAt(0) { it -> it is RetrofitResponse.Remote.Failure<ResponseBody> }
        testObserver.assertValueAt(0) { it -> (it as RetrofitResponse.Remote.Failure<ResponseBody>).throwable is HttpException }
        testObserver.assertValueAt(0) { it ->
            ((it as RetrofitResponse.Remote.Failure<ResponseBody>).throwable as HttpException)
                    .code() == 404
        }
        testObserver.assertValueAt(1) { it -> it is RetrofitResponse.Local.Success<ResponseBody> }
        testObserver.assertValueAt(1) { it -> (it as RetrofitResponse.Local.Success<ResponseBody>).data.string() == responseBody }
    }
