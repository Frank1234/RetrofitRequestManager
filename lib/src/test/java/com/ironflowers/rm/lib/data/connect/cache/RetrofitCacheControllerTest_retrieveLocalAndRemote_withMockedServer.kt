...
class RetrofitCacheControllerTest_retrieveLocalAndRemote_withMockedServer {
    ...
    fun localSuccessRemoteSuccess() {
        ...
        val testObserver = cacheController
                .retrieveLocalAndRemote { cachingHeader -> exampleCachingApi.doSomeExampleGet(cachingHeader) }
                .test()
        ...
        testObserver.assertValueAt(0) { it -> it is RetrofitResponse.Local.Success<ResponseBody> }
        testObserver.assertValueAt(0) { it -> (it as RetrofitResponse.Local.Success<ResponseBody>).data.string() == responseBody }
    ...
    fun localFailureRemoteFailure() {
        ...
        testObserver.assertValueAt(0) { it ->
            ((it as RetrofitResponse.Local.Failure<ResponseBody>).throwable as HttpException)
                    .code() == 504 // 504 is no cache found
        }
        testObserver.assertValueAt(1) { it -> it is RetrofitResponse.Remote.Failure<ResponseBody> }
        testObserver.assertValueAt(1) { it -> (it as RetrofitResponse.Remote.Failure<ResponseBody>).throwable is HttpException }
        testObserver.assertValueAt(1) { it ->
